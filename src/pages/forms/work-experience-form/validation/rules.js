...
export const formRules = {
  step1: {
    ...
    age: [{ required: true, message: '年龄必填' }, { number: true, message: '年龄必须是数字' }, { min: 1, message: '年龄不能小于1' }],
    phoneNumber: [{ required: true, message: '手机号必填' }, { pattern: /^1[3-9]\d{9}$/, message: '手机号格式不正确' }],
    email: [{ required: true, message: '邮箱必填' }, { email: true, message: '邮箱格式不正确' }],
    gender: [{ required: true, message: '性别必选' }], // 补充
    region: [{ required: true, message: '所在地区必填' }], // 补充
  },
  step2: {
    highestEducation: [{ required: true, message: '最高学历必选' }],
    graduationYear: [{ required: true, message: '毕业年份必填' }, { number: true, message: '毕业年份必须是数字', trigger: 'blur' }, { min: 1900, message: '毕业年份不能早于1900', trigger: 'blur' }],
  },
  step3: {
    primaryInterest: [{ required: true, message: '主要兴趣必选' }],
    hasLearningGoal: [{ required: true, message: '请选择是否有明确学习目标', trigger: 'change', validator: (val, { formData }) => {
        // 只有当 shouldShowLearningGoalFields 成立时才要求 hasLearningGoal 必填
        // 这里只是一个形式，实际判断由 customValidationRules 中的 isLearningGoalFilled
        return true;
    }}],
    specificLearningGoal: [{ required: true, message: '具体学习目标必填', trigger: 'blur', validator: (val, { formData }) => {
        // 只有当 hasLearningGoal 为 true 时才要求 specificLearningGoal 必填
        // 这个校验由 customValidationRules 中的 isLearningGoalFilled 负责
        return true;
    }}],
  },
  step4: {
    selectedCourses: [{ required: true, message: '请至少选择一门课程', trigger: 'change', validator: (val) => val && val.length > 0 }],
  },
};
...
export const workExperienceRules = {
  ...
  workExperienceYears: [{ required: true, message: '工作年限必填', trigger: 'blur' }, { number: true, message: '工作年限必须是数字', trigger: 'blur' }, { min: 0, message: '工作年限不能为负', trigger: 'blur' }],
};
  ...
  2: [
    {
      ...
      validator: (store) => {
        ...
        if (age < 18 && ['大专', '本科', '硕士', '博士'].includes(highestEducation)) {
            MessagePlugin.error(customValidationRules[2].find(r => r.name === 'ageEducation').message);
            return false;
        }
        return true;
      }
        ...
        validator: (store) => {
            ...
            store.step2.workExperiences.forEach((exp, index) => {
                ...
                if (store.shouldShowEmploymentFields(index)) {
                    ...
                    if (exp.isEmployed === true && (!exp.companyName || !exp.jobTitle || exp.workExperienceYears === null || exp.workExperienceYears < 0)) {
  ...
  4: [
    {
      ...
      validator: (store) => {
        ...
        if (['小学', '初中'].includes(highestEducation)) {
            const restrictedCourses = ['高级数据分析实战', '资深项目管理认证', '企业战略管理', '高管领导力提升', '大数据处理与分析', 'Java全栈开发', '产品经理实战', '高级英语口语'];
