<template>
  <t-form-item label="推荐课程">
    <t-alert
      v-if="store.step4.recommendedCourses.length === 0"
      theme="info"
      message="根据您的信息，暂无推荐课程。您可以手动选择。"
      class="alert-message"
    />
    <t-checkbox-group v-else v-model="form.selectedCourses" :min="1" @change="calculateAndSetTotalPrice">
      <t-space>
        <div v-for="course in store.step4.recommendedCourses" :key="course" class="course-item">
          <t-checkbox :value="course" :disabled="isRestrictedCourse(course)">
            {{ course }}
  ...
  <t-form-item label="手动选择课程">
    ...
    <t-alert
      v-if="!isCourseSelectionValidByEducation"
      theme="error"
      :message="`您当前的学历（${store.step2.highestEducation}）不支持选择某些高级课程。`"
      class="error-alert"
    />
  </t-form-item>

  <t-form-item v-if="store.shouldShowMaterialPackOption" label="学习资料包">
  ...
  <t-form-item v-if="store.shouldShowDiscountCode" label="优惠码">
    <t-input
      v-model="form.discountCode"
      placeholder="例如：STUDENT80 或 EMPLOYEE90 或 NEWUSER75"
      @change="calculateAndSetTotalPrice"
    ></t-input>
    <p class="t-form__help">
      提示：学生（大专及以上学历）可使用 **STUDENT80** 享受8折；在职人士可使用 **EMPLOYEE90** 享受9折；新用户可使用
      **NEWUSER75** 享受7.5折。
    </p>
  </t-form-item>
...
// 使用课程逻辑 Hook
const { calculateAndSetTotalPrice, isRestrictedCourse } = useCourseLogic(store);
