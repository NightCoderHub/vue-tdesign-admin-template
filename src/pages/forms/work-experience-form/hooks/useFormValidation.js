...
export function useFormValidation(store, formRef) {

  // TDesign 表单的规则
  const currentStepRules = computed(() => {
    switch (store.currentStep) {
      case 1: return formRules.step1;
      case 2: return formRules.step2;
      case 3: return formRules.step3;
      case 4: return formRules.step4;
      default: return {};
    }
  });
  ...
  const currentStepData = computed(() => {
    switch (store.currentStep) {
      case 1: return store.step1;
      case 2: return store.step2;
      case 3: return store.step3;
      case 4: return store.step4;
      default: return {};
    }
  });
  ...
  const validateCustomRules = (step) => {
    ...
    for (const rule of customValidationRules[step]) {
      if (!rule.validator(store)) { // 传入 store 访问全局状态
